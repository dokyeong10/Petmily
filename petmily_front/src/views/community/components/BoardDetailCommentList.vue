      ...
      <div v-if="replyJoins">
        ...
        <div v-for="(reply, idx) in replyJoins" :key="idx+'a'" class="p-1">
          ...
          <div v-if="reply.user === null" class="d-flex justify-content-start">
            <div align="left">
              <div class="d-flex justify-content-start">
                ...
                <div class="d-flex align-items-center p-2"> {{ reply.regdate.substring(0,10) }} </div>
                <div v-if="reply.agency.agencycode == agencycode" class="d-flex align-items-center p-2">
          ...
          <div v-if="reply.toggle">
            <div v-for="(comment, idx) in comments" :key="idx+'c'" class="d-flex justify-content-start mx-auto" style="width: 80%">
              <div v-if="comment.grp === reply.grp">
                <div v-if="comment.user != null">
                  <span class="font-bold pb-3">{{ comment.user.username }} : </span>
                  <span> {{ comment.contents }} </span>
                  <button @click="commentDelete(idx)" v-if="comment.user.no === userno" class="bt-white btn-sm ms-2"> 삭제 </button>
                </div>
                <div v-else>
                  <span class="font-bold pb-3">{{ comment.agency.username }} : </span>
                  <span> {{ comment.contents }} </span>
                  <button @click="commentDelete(idx)" v-if="comment.agency.agencycode == agencycode" class="bt-white btn-sm ms-2"> 삭제 </button>
                </div>
              </div>
            </div>
            <br>
            ...
            <div class="mx-auto" style="width: 80%;">
              ...
              <div class="d-flex justify-content-end">
                <button
                @click="commentInput(idx,reply)"
                class="bt-white btn-sm"
                  >답글 입력하기</button>
              </div>
            </div>
            <hr>
